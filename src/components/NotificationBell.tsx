...
export default function NotificationBell() {
  ...
  const handleNotificationClick = async (notification: typeof notifications[0]) => {
    try {
      ...
      if (isNavigating || !notification.syor_id) return
      ...
      // Small delay before navigation to ensure dropdown closes
      await new Promise(resolve => setTimeout(resolve, 150))
      
      // Navigate to syor detail
  ...
  const getTimeAgo = (dateString: string) => {
    ...
    return past.toLocaleDateString('ms-MY')
  }
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'overdue':
        return (
          <div className={`w-10 h-10 rounded-full ${type === 'overdue' ? 'bg-red-500/20' : 'bg-orange-500/20'} flex items-center justify-center flex-shrink-0`}>
            <svg className={`w-5 h-5 ${type === 'overdue' ? 'text-red-400' : 'text-orange-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <>
      ...
      {/* Dropdown Panel - Using Portal to render outside header */}
      {isOpen && mounted && createPortal(
        <>
          <div 
            ref={dropdownRef}
            className="fixed w-96 bg-slate-800 border border-slate-700 rounded-lg shadow-2xl max-h-[600px] flex flex-col"
            style={{
              zIndex: 99999,
                ...
                {notifications.map((notification) => (
                  <div
                    ...
                    className={`p-4 hover:bg-slate-700/50 transition-colors ${
                      isNavigating ? 'cursor-wait opacity-50' : 'cursor-pointer'
